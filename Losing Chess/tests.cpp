...
void testBoard(istream &is, int board_id) {
    ...
    for (size_t i = 0; i < 8; ++i) {
        ...
        board_ss << line.substr(0, 8) << "\n"; //IMPORTANT!!! Needed to add new line otherwise creation of board became messed up! It would
        //only take the first row 8 times!
    }
...
int main() {
    try {
        ...
        int board_id = 1;
        while (cin.peek() != EOF) { //orignal cin.eof() does not work for me on Linux or Mac!
